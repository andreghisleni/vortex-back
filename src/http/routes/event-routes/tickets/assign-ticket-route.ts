...
export const assignTicketRoute = new Elysia().macro(authMacro).post(
  ...
  async ({ params, body, set, user }) => {
    ...
    if (body.allocationId) {
      const alloc = await prisma.memberTicketAllocation.findUnique({ where: { id: body.allocationId } });
    ...
    await prisma.$transaction([
      prisma.ticket.update({ where: { id: ticket.id }, data: { memberId: body.memberId } }),
      prisma.ticketFlow.create({ data: { ticketId: ticket.id, eventId: params.eventId, type: "ASSIGNED", fromMemberId: null, toMemberId: body.memberId, performedBy: user?.id ?? null } }),
    ]);
  ...
  },
  {
    ...
    detail: { summary: "Assign a ticket to a member (and optional allocation)", operationId: "assignTicket" },
  }
);
