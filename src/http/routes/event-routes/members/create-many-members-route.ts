...
const createMemberWithSessionNameBodySchema = t.Object({
  ...
  ticketAllocations: t.Optional(t.Array(t.Object({
  ...
  .post(
    ...
    async ({ params, body, set }) => {
      ...
      if (event.autoGenerateTicketsTotalPerMember == null) {
        ...
        for (const item of b) {
          ...
          if (!Array.isArray(allocations)) {
            set.status = 400;
            return { error: 'Each member must include ticketAllocations when event.autoGenerateTicketsTotalPerMember is not set' };
          }
          ...
          if (rangeIds.length !== providedIds.length || JSON.stringify(rangeIds) !== JSON.stringify(providedIds)) {
            set.status = 400;
            return { error: 'Each member ticketAllocations must include one entry for each ticket range of the event' };
          }
        }
      }
      ...
      const createOps = inputs.map((item) => {
        ...
        const session = sessions.find((s) => s.name.toLowerCase() === sessionName.toLowerCase());
        if (!session) {
          // Deveria ter sido validado anteriormente, mas checamos por segurança
          throw new Error(`Session not found for name ${sessionName}`);
        }

        const visionId = typeof item.visionId === 'string' && item.visionId !== 'undefined' ? item.visionId : null;
      ...
      const allAllocations: { memberId: string; eventTicketRangeId: string; quantity: number }[] = [];
      for (let i = 0; i < createdMembers.length; i++) {
        const member = createdMembers[i];
        const allocations = inputs[i].ticketAllocations;
        if (Array.isArray(allocations)) {
          for (const a of allocations) {
            allAllocations.push({ memberId: member.id, eventTicketRangeId: a.eventTicketRangeId, quantity: a.quantity });
          }
        }
      }

      if (allAllocations.length > 0) {
        await prisma.memberTicketAllocation.createMany({ data: allAllocations, skipDuplicates: true });
      }
